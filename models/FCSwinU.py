...
class SpectralTransform(nn.Module):
    def __init__(self, embed_dim, last_conv=False):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(embed_dim // 2),  # 添加BN层
...
class FSwinU(nn.Module):
    def __init__(self, img_size,hsi_chans=191, num_feature=201,scale_factor= 8):
        super(FSwinU, self).__init__()
        hsi_chans = hsi_chans
        num_feature = num_feature
        img_size = img_size
        self.scale_factor = scale_factor
        ####################
        self.SSFC = SSFC(hsi_chans)
        self.sunet = SUNet(img_size=img_size, patch_size=4, in_chans=34, out_chans=num_feature,
                           embed_dim=96, depths=[2, 2, 2, 2],
                           num_heads=[8, 8, 8, 8],
                           window_size=4, mlp_ratio=4., qkv_bias=True, qk_scale=2,
                           drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                           norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
        ...
        self.refine = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(num_feature, hsi_chans, 3, 1, 1)
        )
    def UP(self,HSI):
    ...
    def forward(self, HSI, MSI):
        ...
        UP_LRHSI_fea = self.SSFC(UP_LRHSI)
        Data = torch.cat((UP_LRHSI_fea, MSI), 1)
        Data = self.sunet(Data)
...
if __name__ == '__main__':
    ...
    LRHSI = torch.randn((1, 31, 64, 64))
    HRMSI = torch.randn((1, 3, 512, 512))
    # wdcm
    # LRHSI = torch.randn((1, 191, 8, 8))
    # LRHSI = torch.randn((1, 191, 16, 16))
    # HRMSI = torch.randn((1, 10, 128, 128))
    # #dc
    # LRHSI = torch.randn((1, 93, 8, 8))
    # LRHSI = torch.randn((1, 93, 16, 16))
    # HRMSI = torch.randn((1, 4, 128, 128))
    ...
    # #pu
    model = FSwinU(img_size=imgsize, hsi_chans=31, num_feature=48,scale_factor=8)
    print(model)
