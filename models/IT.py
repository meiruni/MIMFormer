...
class FirstPatchEmbed(nn.Module):
    ...
    """
    def __init__(self, kernel_size=3,  stride=3, padding=1, in_chans=64, embed_dim=768):
        super().__init__()
        
        self.proj1 = nn.Conv2d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding )
        self.norm1 = nn.BatchNorm2d(embed_dim)
        self.gelu1 = nn.GELU()

    def forward(self, x):
        # B, C, H, W = x.shape #[2,64,64,64]
        x = self.proj1(x)
        x = self.norm1(x) #[2,48,32,32]
        x = self.gelu1(x)
        x = x.permute(0,2,3,1)
        return x


class FirstPatchUnembed(nn.Module):
    """ Patch Embedding to 2D Image
    """
    def __init__(self, img_size=64, kernel_size=8, stride=8, padding=1, in_chans=96, embed_dim=768):
        super().__init__()

        self.proj1 = nn.ConvTranspose2d(embed_dim, in_chans, kernel_size=kernel_size, stride=stride)
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=64, patch_size=8, in_chans=64, num_classes=1000, embed_dims=None, depths=None,
        ...
        self.patch_embed = FirstPatchEmbed(in_chans=in_chans, embed_dim=embed_dims[0],kernel_size=patch_size,stride=patch_size)
        self.num_patches1 = num_patches = img_size // patch_size
        ...
        self.patch_unembed = FirstPatchUnembed(in_chans=in_chans, embed_dim=embed_dims[0],kernel_size=patch_size,stride=patch_size)
