...
class PSRTnet(nn.Module):
    def __init__(self, hsi_channels=31, msi_channels=10,scale_factor= 8):
        super(PSRTnet, self).__init__()
        # self.args = args
        self.img_size = 64
        self.in_channels = hsi_channels
        self.embed = 36
        ...
        self.w = Block(out_num=2, inside_num=3, img_size=self.img_size, in_chans=hsi_channels+msi_channels, embed_dim=self.embed, head=6,
                       win_size=8)
...
if __name__ == '__main__':
    ...
    LRHSI = torch.randn((2, 191, 8, 8)).to('cuda:0')
    HRMSI = torch.randn((2, 10, 64, 64)).to('cuda:0')
    # wdcm
    ...
    model = PSRTnet(hsi_channels=191, msi_channels=10,scale_factor= 8).to('cuda:0')
    print(model)
