...
class Downsample(nn.Module):
    def __init__(self, n_channels, ratio):
        ...
        for i in range(int(np.log2(ratio))):
            dconvs.append(
                nn.Conv2d(n_channels, n_channels, 3, stride=2, padding=1, dilation=1, groups=n_channels, bias=True))
...
class Upsample(nn.Module):
    def __init__(self, n_channels, ratio):
        ...
        for i in range(int(np.log2(ratio))):
            uconvs.append(
                nn.ConvTranspose2d(in_channels=n_channels, out_channels=n_channels, kernel_size=3, stride=2, padding=1,
                                   output_padding=1))
...
class DCT(nn.Module):
    def __init__(self, n_colors,msi_chans, upscale_factor, n_feats=180):
        ...
        self.headY = nn.Sequential(
            nn.Conv2d(msi_chans, 64, kernel_size, stride=1, padding=3 // 2),
            nn.ReLU(),
...
if __name__ == '__main__':
    ...
    # # wdcm
    # LRHSI = torch.randn((1, 191, 16, 16)).to('cuda:0')
    # HRMSI = torch.randn((1, 10, 128, 128)).to('cuda:0')

    # #pu
    LRHSI = torch.randn((1, 93, 8, 8)).to('cuda:0')
    HRMSI = torch.randn((1, 4, 64, 64)).to('cuda:0')
    ...
    # #pu
    model = DCT(n_colors=93,msi_chans =4 ,upscale_factor=8).to('cuda:0')
    print(model)
